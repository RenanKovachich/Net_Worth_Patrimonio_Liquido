...
app_ = Label(frameCima, text="Calculadora de patrimônio líquido", compound=LEFT, padx=5, relief=FLAT, anchor=NW,
...
# Entrando Ativos -----------------------------------------------------------------------
l_nome = Label(frameAtivos, text="Insira Ativos", padx=10, width=35, height=1, anchor=NW, font='Verdana 11', bg=co2,
               fg=co1)
...
l_investmentos = Label(frameAtivos, text='Investimentos', height=1, anchor=E, font='Verdana 9', bg=co9, fg=co0)
l_investmentos.place(x=10, y=230)
...
l_nome = Label(framePassivos, text="Insira Passivos", padx=10, width=35, height=1, anchor=NW, font='Verdana 11',
               bg=co5, fg=co1)
l_nome.place(x=0, y=0)
...
# Outras Dívidas
l_divida = Label(framePassivos, text="Outras dívidas", height=1, anchor=E, font="Verdana 9", bg=co9, fg=co0)
l_divida.place(x=10, y=230)
...
def calcular():
    ...
    if ativo_1 == '' or ativo_2 == '' or ativo_3 == '' or ativo_4 == '' or ativo_5 == '' or passivo_1 == '' or \
            passivo_2 == '' or passivo_3 == '' or passivo_4 == '':
...
# Resultado -----------------------------------------------------------------------
l_resultado = Label(frameResultado, text='R${:,.2f}'.format(00), padx=10, width=15, height=1, anchor=NE,
                    font="Verdana 25 bold", bg=co3, fg=co1)
l_resultado.place(x=0, y=7)

botao_calcular = Button(framePassivos, command=calcular, width=12, anchor=CENTER, text=" Calcular".upper(),
                        overrelief=RIDGE, font="Ivy 9 bold", bg=co1, fg=co0)
